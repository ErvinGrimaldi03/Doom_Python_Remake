...
class SegHandler:
    ...
    def draw_solid_wall_range(self, x1, x2):
        ...
        renderer = self.engine.view_render
    ...
    def draw_portal_wall_range(self, x1, x2):
        ...
        line = seg.linedef
        side = seg.linedef.front_sidedef
        render = self.engine.view_render
        upper_clip = self.upper_clip
        lower_clip = self.lower_clip
        ...
        for x in range(x1, x2 + 1):
            ...
            if b_draw_upper_wall:
                ...
                if b_draw_ceil:
                    cy1 = upper_clip[x] + 1
                    cy2 = int(min(draw_wall_y1 - 1, lower_clip[x] - 1))
                    render.draw_vline(x, cy1, cy2, tex_ceil_id, light_level)
                #
                wy1 = int(max(draw_upper_wall_y1, upper_clip[x] + 1))
                wy2 = int(min(draw_upper_wall_y2, lower_clip[x] - 1))
                render.draw_vline(x, wy1, wy2, upper_wall_texture, light_level)
                #
                if upper_clip[x] < wy2:
                    upper_clip[x] = wy2
                #
            ...
            if b_draw_lower_wall:
                ...
                if b_draw_floor:
                    fy1 = int(max(draw_wall_y2 + 1, upper_clip[x] + 1))
                    fy2 = lower_clip[x] - 1
                    render.draw_vline(x, fy1, fy2, tex_floor_id, light_level)
                #
                ...
                render.draw_vline(x, wy1, wy2, lower_wall_texture, light_level)
                #
                if lower_clip[x] > wy1:
                    lower_clip[x] = wy1
                #
